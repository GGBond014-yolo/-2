...
function startHold() {
  ...
  holdTimer = window.setTimeout(() => {
    ...
    acceptButton.textContent = '邀请已接受 ✓'
...
const fishBlessings = ['新婚快乐，百年好合', '早生贵子，幸福美满', '愿你们相爱到永远', '祝新人甜甜蜜蜜', '愿岁月可回首，且以深情共白头']
let caught = 0
...
for (let index = 0; index < 9; index += 1) {
  ...
  fish.addEventListener('click', () => {
    ...
    if (caught === 5) {
      const message = document.createElement('p'); message.className = 'fish-unlock'; message.textContent = '祝福已全部收到：幸福值 +1000'; fishField.append(message)
    }
  })
...
if (publicRsvpUrl) {
  ...
  const storageKey = 'wedding-rsvp-xu-cai-2026'
  let savedRsvp
  ...
  function updateAccommodation() {
    ...
    if (needed) { rsvpForm.elements.checkInAt.value ||= '2026-10-24T14:00'; rsvpForm.elements.checkOutAt.value ||= '2026-10-25T12:00' }
  }
  function fillRsvp(data) {
    ...
    rsvpForm.elements.checkInAt.value = data.checkInAt || '2026-10-24T14:00'; rsvpForm.elements.checkOutAt.value = data.checkOutAt || '2026-10-25T12:00'; messageCount.value = String(rsvpForm.elements.message.value.length); updateAccommodation()
  }
  ...
  rsvpForm.addEventListener('submit', async (event) => {
    ...
    rsvpSubmit.disabled = true; rsvpSubmit.querySelector('span').textContent = '正在保存登记信息……'
    try {
      savedRsvp = { ...submission, id: 'demo-local-only', editToken: undefined }
      localStorage.setItem(storageKey, JSON.stringify(savedRsvp))
      rsvpForm.hidden = true; rsvpSuccess.hidden = false; rsvpSuccessTitle.textContent = `${submission.guestName}，登记成功`; rsvpSuccessSummary.textContent = `已在本机保存 ${submission.partySize} 人的出席记录${submission.needsAccommodation ? ' · 已登记住宿需求' : ' · 无需住宿'}；这些内容仅保存在当前浏览器中。`; rsvpSuccess.focus({ preventScroll: true })
